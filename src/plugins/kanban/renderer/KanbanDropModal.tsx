...
export interface KanbanDropPayload {
  title: string
  description?: string
  clientX: number
  clientY: number
...
export function KanbanDropModal({ payload, onClose }: Props): React.ReactElement {
  const { title, description, clientX, clientY } = payload
  ...
  const handleStart = useCallback(async (agentId: string) => {
    ...
    if (agentId === 'orchestrate') {
      ...
      try {
        ...
        console.error('[KanbanDropModal] orchestrate error:', e)
      } finally {
        setLoading(false)
      }
      return
    ...
    try {
      ...
      console.error('[KanbanDropModal]', e)
    } finally {
      setLoading(false)
    }
  }, [autoBranch, branchBlocked, branchName, title, description, getWorldPos, workspace?.path, onClose])
